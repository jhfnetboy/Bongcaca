...
## 2024-04-14 (0.23.41)
- 改进：将"Select Language"改为"Translate your voice to"，使意图更加明确
- 新增：添加泰语(Thai)作为支持的语言选项
- 修复：解决Intel Mac上出现的"unknown opcode"错误问题
- 优化：改进绘制函数，使用整数坐标和异常处理，提高跨平台兼容性
- 修复：应用图标在MacOS中的显示问题，确保应用在dock中有正确的图标
- 改进：简化空闲状态波形显示逻辑，减少不必要的资源使用

## 2024-04-14 (0.23.40)
- 修复：解决应用图标在MacOS中不显示的问题
- 修复：修复关闭窗口不退出应用的问题，添加系统托盘图标及退出菜单
- 改进：优化语言选择功能，根据所选模型动态更新可用语言选项
- 改进：确保语言选择立即生效，不再需要等待下次操作
- 技术：添加系统托盘支持，使应用图标在dock栏正确显示

## 2024-04-13 (0.23.39)
- 修复：解决了WindowSignals类缺失导致的导入错误
- 改进：优化菜单项透明度，设置为60%透明度的白色背景，改善菜单项可视性
- 技术：添加了与主应用通信的信号类定义
- 优化：调整样式表，提高用户体验
...
- 添加 `get_language` 方法到 FloatingWindow 类，方便获取当前选择的语言
- 优化语言选择功能，支持更好的语言代码读取
