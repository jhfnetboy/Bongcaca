...
- 🎉 完成录音和语音识别核心功能修复
- 修复：解决VAD过滤器过于严格导致所有音频被过滤的问题
- 优化：降低无语音检测阈值从0.6到0.3，减少最小静音时长从500ms到200ms
- 改进：调整VAD参数threshold从0.5到0.3，适配虚拟音频设备和低音量输入
- 修复：改进音频电平计算算法，使用RMS和对数缩放提高低音量可见性
- 优化：增强录音设备检测和初始化，支持多种音频配置自动尝试
- 改进：添加录音参数日志输出，便于调试音频问题
- 修复：优化转写结果验证逻辑，提供更友好的错误提示
- 技术：保持模型加载状态，避免每次转写重新加载模型
- 修复：解决hotkey_listener模块缺失导入的问题
- 标记：准备进入代码优化和重构阶段
