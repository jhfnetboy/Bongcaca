...
#### new
INFO - 录音统计: 文件大小=0.22MB, 录音时长=7.35秒, 转写时间=22.62秒, 字符数=34, 中文字数=21, 使用模型=medium, 语言=auto
 auto的意思是默认中文输入，中文输出

 去掉所有提示音，然后当设备检测并准备完毕，模型加载完毕，绿色三角start被点击后，三者都具备，则使用beep轻轻嘟一声，表示可以说话了
而当用户点击stop红色方块后，文件开始存储，转写，如果撰写完成并复制到了内存，请beep轻轻的滴滴两声，表示完成
